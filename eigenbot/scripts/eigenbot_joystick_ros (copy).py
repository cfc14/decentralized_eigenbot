...
def joint_state_talker(module_types=module_types_test,
    ...
    # FOR DEBUG
    # const_offsets = const_offsets*0

### MAIN LOOP
    if SEND_COMMANDS:
        pub = rospy.Publisher('/eigenbot/joint_cmd', JointState, queue_size=10)
    ...
    while not rospy.is_shutdown():
        ...
        # amplitudes[0:3] = amplitude_max*np.ones(3)*(forward_cmd)
        # amplitudes[3:6] = amplitude_max*np.ones(3)*(forward_cmd)
        ...
        if np.linalg.norm(axes[0:2])>0.01:
            wave_phase += dt
        # if we are not moving, interpolate to wave phase zero
